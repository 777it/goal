...
class DayListViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let goals = userDefaults.stringArray(forKey: "KEY_Goal_day")
        if (goals != nil) {
            for goal in goals! {
                myItems.append(goal)
            }
    ...
    class func addGoal(str:String) {
        ...
        let userDefaults:UserDefaults = UserDefaults.standard
        let goals = userDefaults.array(forKey: "KEY_Goal_day")
        
        if (goals == nil) {
            userDefaults.set([myNewGoal], forKey: "KEY_Goal_day")
            userDefaults.set([""], forKey: "KEY_Goal_day_give")
            userDefaults.set([""], forKey: "KEY_Goal_day_take")
        }
        else {
            var strGoals = userDefaults.array(forKey: "KEY_Goal_day") as! [String]
            ...
            userDefaults.set(strGoals, forKey: "KEY_Goal_day")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // 移動先を定義する.
        let dayDetailViewController = DayDetailViewController()
        // 選択された値を移動先にセット
        dayDetailViewController.myGoal = myItems[indexPath.row]
        dayDetailViewController.myGoalNum = indexPath.row
